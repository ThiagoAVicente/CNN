...
from src.functions import con2d, dense, max_pooling, relu, relu_derivate, softmax
...
class cnn:
    ...
    def backward_propagation(self, true_labels: np.ndarray):
        ...
        dz_relu *= relu_derivate(self.features)
            ...
            for j in range(self.features.shape[1]):
                for k in range(self.features.shape[2]):
                    region = self.input_image[
                        j : j + self.filter_size, k : k + self.filter_size
                    ]
